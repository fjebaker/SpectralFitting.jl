...
add_models(_, _, ::M1, ::M2) where {M1,M2} = error("Left and right models must be Additive.")
...
mult_models(_, _, ::M1, ::M2) where {M1,M2} = error("Left model must be Multiplicative and right model must be Additive.")
mult_models(m1::M1, m2::M2) where {M1,M2} = mult_models(m1, m2, modelkind(M1), modelkind(M2))
function Base.:*(m1::M1, m2::M2) where {M1<:AbstractSpectralModel,M2<:AbstractSpectralModel}
    mult_models(m1, m2, modelkind(M1), modelkind(M2))
end

conv_models(m1, m2, ::Convolutional, ::Additive) = CompositeSpectralModel(m1, m2)
conv_models(_, _, ::M1, ::M2) where {M1,M2} = error("Left model must be Convolutional and right model must be Additive.")
conv_models(m1::M1, m2::M2) where {M1,M2} = conv_models(m1, m2, modelkind(M1), modelkind(M2))
