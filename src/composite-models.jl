...
"""
    CompositeSpectralModel{M1,M2,O} <: AbstractSpectralModel
...
`CompositeSpectralModel`, where the operation is given by an [`AbstractCompositeOperator`](@ref).
...
function invokemodel(e, m::CompositeSpectralModel)
    fluxes = make_fluxes(e, flux_count(m))
    invokemodel!(fluxes, e, m)
    first(fluxes)
end
function invokemodel(e, m::CompositeSpectralModel, free_params)
...
function get_params_value(model::CompositeSpectralModel)
    T = generated_get_model_number_type(model)
    params = T[]
...
function get_params(model::CompositeSpectralModel)
...
function get_param_symbol_pairs(model::CompositeSpectralModel)
...
function get_param(model::CompositeSpectralModel, s::Symbol)
...
function _expression_string(cm::CompositeSpectralModel{M1,M2}) where {M1,M2}
    left = _expression_string(cm.left)
    right = _expression_string(cm.right)
    _expression_string(left, right, modelkind(M1))
end

function Base.show(io::IO, ::CompositeSpectralModel)
    print(io, "CompositeSpectralModel")
end

# such an ugly function
function _printinfo(io::IO, model::CompositeSpectralModel{M1,M2}) where {M1,M2}
    l_buffer = 5
    model_types = generated_get_model_types(model)
    n_components = length(model_types)
    expr, infos = _readable_expression_info(model)

    print(io, "CompositeSpectralModel with $n_components component models:\n")
    println(
        io,
...
function _readable_expression_info(model::CompositeSpectralModel)
